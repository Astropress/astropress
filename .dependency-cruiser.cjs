...
module.exports = {
	forbidden: [
		...
		// that the runtime layer consumes, not the other way around.
		{
			...
			severity: "error",
			comment:
				"Adapter files must not import from runtime-* files (adapter layer cannot depend on runtime layer)",
		...
		// public API or file-system analysis).
		{
			...
			severity: "error",
			comment:
				"Tooling scripts must not import from package source — use file-system analysis or the public API",
