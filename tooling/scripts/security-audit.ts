...
async function main() {
  ...
  for (const file of auditedFiles) {
    ...
    if (/<script\s+is:inline\b/i.test(content)) {
      violations.push({ file: display, message: "inline script remains; this weakens CSP enforcement" });
    }

    if (/\son[a-z]+=/i.test(content)) {
      violations.push({ file: display, message: "inline event handler attribute found" });
    }

    if (/contenteditable=/i.test(content)) {
      violations.push({ file: display, message: "contenteditable usage found in audited admin/auth surface" });
    }

    if (/set:html=\{pageRecord\.body\}/.test(content)) {
      violations.push({ file: display, message: "unsafe post body preview render path found" });
    }

    if (/\binnerHTML\s*=/.test(content)) {
      violations.push({ file: display, message: "direct innerHTML assignment found" });
    }
  }
