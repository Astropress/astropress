...
export const ADMIN_SMOKE_ROUTES = [
	...
	// Stub leaves added in IA restructure — every leaf renders unconditionally
	// and the page itself shows <RequiresIntegration> when no provider is set.
	"/ap-admin/forms",
	"/ap-admin/newsletter",
	"/ap-admin/events",
	"/ap-admin/reviews",
	...
	try {
		...
		server = spawnServer(
			...
			[
				"run",
				"--filter",
				"astropress-example-npm-consumer-smoke",
				"dev",
				"--",
				"--host",
				"127.0.0.1",
		...
		if (failures.length > 0) {
			console.error(
				`consumer-smoke failed — ${failures.length} route(s) did not return HTTP 200:\n`,
			);
		...
		console.log(
			`consumer-smoke passed — all ${ADMIN_SMOKE_ROUTES.length} routes returned HTTP 200.`,
		);
	} finally {
