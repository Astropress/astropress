...
function getSymbolKind(
	symbol: ts.Symbol,
): "function" | "type" | "interface" | "const" | "class" | "enum" {
...
if (CHECK_MODE) {
	if (!existsSync(OUT_FILE)) {
		console.error(
			"docs:api:check failed — docs/reference/API_REFERENCE.md does not exist. Run `bun run docs:api`.",
		);
		process.exit(1);
	}
	...
	if (normalize(existing) !== normalize(output)) {
		console.error(
			"docs:api:check failed — docs/reference/API_REFERENCE.md is stale. Run `bun run docs:api`.",
		);
		process.exit(1);
	}
	...
	console.log(
		`✓ API reference written to docs/reference/API_REFERENCE.md (${allEntries.length} entries)`,
	);
}
