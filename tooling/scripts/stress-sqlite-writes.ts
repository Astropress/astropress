...
`.trim(),
	...
	const getArg = (flag: string, def: number): number => {
		...
		return match ? Number.parseInt(match.split("=")[1] ?? "", 10) : def;
	};
	...
	console.log(
		`  Writes      : ${writesPerWorker} per worker (${concurrency * writesPerWorker} total)`,
	);
  ...
  `);
	...
	const rowCount = (
		verifyDb.prepare("SELECT COUNT(*) as n FROM stress_writes").get() as {
			n: number;
		}
	).n;
	verifyDb.close();
	...
	if (passed) {
		...
		if (totalBusy > 0)
			console.log(
				`  ${totalBusy} SQLITE_BUSY errors detected — increase busy_timeout or reduce concurrency`,
			);
		if (totalOther > 0) console.log(`  ${totalOther} unexpected errors`);
		if (rowCount !== concurrency * writesPerWorker) {
			console.log(
				`  Row count mismatch: expected ${concurrency * writesPerWorker}, got ${rowCount}`,
			);
		}
		process.exit(1);
	}
