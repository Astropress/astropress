...
async function main() {
  ...
  for (const filename of componentFiles) {
    ...
    if (!src.includes("customElements.define(\"ap-") && !src.includes("customElements.define('ap-")) {
      violations.push(`${label}: missing customElements.define('ap-...')  — custom element name must use the ap- prefix`);
    }

    // 2. connectedCallback
    if (!src.includes("connectedCallback()") && !src.includes("connectedCallback (){")) {
      violations.push(`${label}: missing connectedCallback — required lifecycle hook`);
    }

    // 3. disconnectedCallback
    if (!src.includes("disconnectedCallback()") && !src.includes("disconnectedCallback (){")) {
      violations.push(`${label}: missing disconnectedCallback — required to prevent memory/listener leaks`);
    }
