...
const OPTIONAL_TEST_DIRS = [
  join(root, "packages/astropress-mcp"),
];
...
const STUB_PATTERNS: StubPattern[] = [
  ...
  { name: "skipped test with body", pattern: /\bit\.skip\s*\(\s*["'][^"']*["']\s*,|\btest\.skip\s*\(\s*["'][^"']*["']\s*,|\bdescribe\.skip\s*\(\s*["'][^"']*["']\s*,/ },
  { name: "vacuous assertion (expect(true))", pattern: /expect\s*\(\s*true\s*\)\s*\.toBe\s*\(\s*true\s*\)/ },
  { name: "vacuous assertion (expect(false))", pattern: /expect\s*\(\s*false\s*\)\s*\.toBe\s*\(\s*false\s*\)/ },
  { name: "constant equality assertion", pattern: /expect\s*\(\s*\d+\s*\)\s*\.toBe\s*\(\s*\d+\s*\)/ },
  { name: "empty test body",    pattern: /\bit\s*\(\s*["'][^"']+["']\s*,\s*\(\s*\)\s*=>\s*\{\s*\}\s*\)/ },
  ...
  { name: "stub comment in test", pattern: /\/\/\s*(TODO|FIXME|placeholder)\b/i },
];
...
async function main() {
  ...
  const allDirs = [
    ...TEST_DIRS,
    ...(await Promise.all(OPTIONAL_TEST_DIRS.map(async (d) => (await directoryExists(d) ? d : null)))).filter(
        ...
        if (typeof pattern === "string") {
          ...
          for (let i = 0; i < lines.length; i++) {
            if (pattern.test(lines[i])) {
              violations.push(`${relPath}:${i + 1}: ${name} — \`${lines[i].trim()}\``);
  ...
  if (violations.length > 0) {
    console.error(`no-stub-tests audit failed — ${violations.length} stub pattern(s) found in ${totalFiles} test files:\n`);
