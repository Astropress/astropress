...
async function main() {
  ...
  const locksPath = join(root, "packages/astropress/src/sqlite-runtime/locks.ts");
  ...
  const lockIndicatorPath = join(root, "packages/astropress/web-components/ap-lock-indicator.ts");
  ...
  try {
    ...
    for (const action of requiredActions) {
      ...
      if (!found) {
        violations.push(
          `[missing-action] No file matching "${action}" found in packages/astropress/pages/ap-admin/actions/ — ` +
          `lock action endpoint is required`,
        );
      }
    }
  } catch {
    for (const action of requiredActions) {
      violations.push(
        `[missing-action] packages/astropress/pages/ap-admin/actions/ directory not found — ` +
        `cannot verify "${action}" endpoint`,
      );
    }
  }

  // 5. Optimistic conflict detection
  const runtimeActionsPath = join(root, "packages/astropress/src/runtime-actions-content.ts");
  ...
  // 7. Content-locking test file exists
  const testPath = join(root, "packages/astropress/tests/content-locking.test.ts");
  ...
  if (violations.length > 0) {
    console.error(`collaboration audit failed — ${violations.length} issue(s):\n`);
