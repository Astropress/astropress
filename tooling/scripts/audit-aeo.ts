...
function auditComponent(name: string, file: string, checks: string[]): void {
  ...
  for (const check of checks) {
    if (!source.includes(check)) {
      failures.push(`MALFORMED: ${name} (${file}) — missing required string "${check}"`);
      exitCode = 1;
    }
  }
}
...
auditComponent(contentLayoutComponent.name, contentLayoutComponent.file, contentLayoutComponent.checks);
...
if (!existsSync(contractsPath)) {
  ...
  for (const requiredType of ["AeoMetadata", "FaqItem", "HowToStep"]) {
    if (!contractsSource.includes(`export interface ${requiredType}`)) {
      failures.push(`MISSING TYPE: ${requiredType} not exported from platform-contracts.ts`);
      exitCode = 1;
    }
  }
}
...
if (failures.length === 0) {
  console.log("✓ audit:aeo — all AEO JSON-LD components present and structurally valid");
  console.log(`  Components audited: ${requiredComponents.length + 1} (${requiredComponents.map((c) => c.name).join(", ")}, ${contentLayoutComponent.name})`);
