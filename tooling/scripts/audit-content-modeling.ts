...
const CONTENT_MODELING_PATH = join(root, "packages/astropress/src/content-modeling.ts");
const PLATFORM_CONTRACTS_PATH = join(root, "packages/astropress/src/platform-contracts.ts");
const TEST_FILE_PATH = join(root, "packages/astropress/tests/content-modeling.test.ts");
...
async function main() {
  ...
  if (!contentModelingExists) {
    violations.push(
      `[missing-file] ${contentModelingRel} does not exist`,
    );
  } else {
    const src = await readFile(CONTENT_MODELING_PATH, "utf8");
    if (!/export\s+(async\s+)?function\s+validateContentFields|export\s*\{\s*[^}]*\bvalidateContentFields\b/.test(src)) {
      violations.push(
  ...
  if (!platformContractsExists) {
    violations.push(
      `[missing-file] ${platformContractsRel} does not exist`,
    );
  } else {
  ...
  if (!testFileExists) {
    violations.push(
      `[missing-test] ${testFileRel} does not exist`,
    );
  }

  if (violations.length > 0) {
    console.error(`content-modeling audit failed — ${violations.length} issue(s):\n`);
