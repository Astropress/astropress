...
async function main() {
  ...
  if (!adminCss.includes("@keyframes")) {
    violations.push("admin.css: no @keyframes block found — loading/motion animations required for interaction feedback");
  }

  // 2. prefers-reduced-motion
  if (!adminCss.includes("prefers-reduced-motion")) {
    violations.push("admin.css: missing prefers-reduced-motion media query — required by WCAG 2.3.3 for users who prefer no motion");
  }

  // 3. Dialog animation (@starting-style or transition on dialog)
  if (!adminCss.includes("@starting-style") && !adminCss.includes("dialog {") && !/dialog\s*\{/.test(adminCss)) {
    violations.push("admin.css: no dialog animation found (@starting-style or dialog transition) — dialogs must animate in/out");
  }

  // 4. At least two web component files with aria-live
  const wcFiles = (await readdir(WC_DIR)).filter((f) => f.endsWith(".ts") && f !== "index.ts");
  let ariaLiveCount = 0;
  ...
  if (noticeSrc) {
    ...
    const hasPositiveGuard = noticeSrc.includes("> 0") || noticeSrc.includes(">= 1") || noticeSrc.includes("!Number.isNaN");
