...
if (!existsSync(CHANGELOG_PATH)) {
	console.error(
		`✗ check:version — package version changed ${baseVersion} → ${currentVersion} but CHANGELOG.md does not exist`,
	);
	process.exit(1);
}
...
if (!changelog.includes(currentVersion)) {
	console.error(
		`✗ check:version — package version changed ${baseVersion} → ${currentVersion} but CHANGELOG.md does not mention ${currentVersion}. Run 'bun run version' (changesets) to update the changelog before bumping the version.`,
	);
	process.exit(1);
}
...
if (existsSync(CARGO_PATH)) {
	...
	if (cargoVersion && cargoVersion !== currentVersion) {
		console.error(
			`✗ check:version — Cargo.toml version (${cargoVersion}) does not match package.json (${currentVersion}). Run 'bun run version' to sync.`,
		);
		process.exit(1);
	}
