...
async function main() {
  ...
  if (security === null) {
    ...
    !/vulnerability|responsible\s+disclosure|report/i.test(security)
  ) {
    violations.push(
  ...
  try {
    ...
    violations.push(
      "[missing] .github/ISSUE_TEMPLATE/ — directory not found",
    );
  }
  ...
  if (violations.length > 0) {
    console.error(
      `oss-health audit failed — ${violations.length} issue(s):\n`,
    );
  ...
  console.log("oss-health audit passed — all open-source health files present and valid.");
}
