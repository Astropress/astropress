...
function extractExportedNames(src: string): string[] {
  ...
  for (const m of src.matchAll(/^export\s+(?:async\s+)?(?:function\*?\s+|class\s+|const\s+)(\w+)/gm)) {
    names.push(m[1]);
  }
  ...
  for (const m of src.matchAll(/^export\s+\{([^}]+)\}(?:\s+from\s+["'][^"']+["'])?/gm)) {
    ...
    for (const part of m[1].split(",")) {
      const name = part.trim().replace(/^type\s+/, "").replace(/\s+as\s+\w+$/, "").trim();
  ...
  for (const filePath of srcFiles) {
    ...
    for (const name of exportedNames) {
      ...
      if (!corpus.includes(name)) {
        violations.push(`${relPath}: \`${name}\` is exported but has no consumers — consider removing or moving to an internal module`);
      }
    }
  }

  if (violations.length > 0) {
    console.error(`dead-exports audit failed — ${violations.length} orphaned export(s):\n`);
