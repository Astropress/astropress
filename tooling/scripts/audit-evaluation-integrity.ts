...
async function main() {
  ...
  const auditScripts = Object.entries(pkgJson.scripts).filter(
  ...
  for (const m of evalSrc.matchAll(rubricLinePattern)) {
    ...
    } else if (evidence.includes("audit:") || evidence.includes("test:") || evidence.includes(".test.ts")) {
      automatedCount++;
    }
  ...
  const selfAssessedPct = totalRubrics > 0 ? Math.round((selfAssessedCount / totalRubrics) * 100) : 0;
