...
test.describe("Feature: admin nav mobile sidebar (<ap-admin-nav>)", () => {
	...
	test("Scenario: toggle click opens the sidebar (data-open set, aria-expanded=true)", async ({
		page,
	}) => {
	...
	test("Scenario: close button closes the sidebar (data-open removed, aria-expanded=false)", async ({
		page,
	}) => {
		const toggle = page.locator("[data-nav-toggle]");
