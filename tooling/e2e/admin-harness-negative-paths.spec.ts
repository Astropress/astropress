...
test.describe("Feature: admin negative-path UX", () => {
	test("Scenario: failed post save keeps the editor open with a specific error", async ({
		page,
	}) => {
	...
	test("Scenario: invalid admin action token redirects with a security-token message", async ({
		page,
	}) => {
		await page.goto("/ap-admin/posts/hello-world", {
			waitUntil: "networkidle",
