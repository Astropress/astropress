...
| 46 | Mobile-Firstness / Responsive Design | A+ | `admin-touch-targets.spec.ts` (strict-compliance gate: every interactive admin target at viewport-375 must meet WCAG 2.5.5 — no per-route baselines, `toEqual([])` enforced via #58); Playwright `viewport-375`, `viewport-768`, `viewport-1280` projects in `test:acceptance` CI |
