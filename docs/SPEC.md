...
2. **Repository URL**: `package.json` points to a non-existent public GitHub repo — must be updated once the GitHub org is created
3. ~~**No automated cloud migration runner**~~: **implemented** — `astropress db migrate --target=d1` shells out to `wrangler d1 execute`; TypeScript `runD1Migrations()` also available
...
5. ~~**PocketBase adapter untested**~~: **implemented** — 12 test cases in `tests/pocketbase-adapter.test.ts`
6. ~~**Neon/Nhost adapters missing**~~: **implemented** — both adapters live in `src/adapters/neon.ts` and `src/adapters/nhost.ts` with full test coverage
7. ~~**API reference from regex**~~: **implemented** — `scripts/generate-api-docs.ts` uses TypeScript compiler API; `bun run docs:api:check` runs in CI

### Important but not blocking

8. **No admin UI import wizard backend**: the import wizard UI exists at `/ap-admin/import/` but the form has no server-side action handler — it shows CLI commands for the user to run instead
9. ~~**No subscriber list management**~~: **implemented** — full CRUD at `/ap-admin/subscribers/` with Listmonk integration
