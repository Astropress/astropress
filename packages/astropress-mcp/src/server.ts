...
const tools: Tool[] = [
	{
		name: "list_content",
		description:
			"List content records. Optionally filter by type (post/page) and status (draft/published/archived).",
		inputSchema: {
			type: "object",
			properties: {
	...
	},
	{
		...
		inputSchema: {
			...
			properties: {
				slug: {
					type: "string",
					description: "URL slug (unique, lowercase, hyphens)",
				},
				...
				status: {
					type: "string",
					description: "draft or published (default: draft)",
				},
			},
		},
	},
	...
	},
	{
		name: "get_revisions",
		description:
			"Get the revision history for a content record. Returns all saved revisions with author, timestamp, and the body snapshot at that point in time.",
		inputSchema: {
			type: "object",
			required: ["id"],
			properties: {
