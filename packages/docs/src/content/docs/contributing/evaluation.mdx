...
Every new project created with `astropress new` receives an `EVALUATION.md` project evaluation card generated from the same 54-rubric framework. That card is intended to be carried forward and updated by downstream Astropress projects, especially when major PRs are substantially AI-generated.
