...
function makeFetchMock(handler: (url: string, init?: RequestInit) => { ok: boolean; status: number; body: unknown }) {
...
describe("POST /jobs/import/wordpress", () => {
  ...
  it("returns 202 with jobId and queued status for known site", async () => {
    vi.stubGlobal("fetch", makeFetchMock(() => ({ ok: true, status: 200, body: { status: "completed" } })));
    const app = createNexusApp({ config: testConfig, authToken: ORG_TOKEN });
    const { status, body } = await callApp(app, "/jobs/import/wordpress", {
      method: "POST",
      token: ORG_TOKEN,
...
describe("GET /jobs/:id", () => {
  ...
  it("returns 404 for unknown job ID", async () => {
    const app = createNexusApp({ config: testConfig, authToken: ORG_TOKEN });
    const { status } = await callApp(app, "/jobs/nonexistent-id", { token: ORG_TOKEN });
    expect(status).toBe(404);
  });
  ...
  it("returns the job entry after it is queued", async () => {
    ...
    const { status, body } = await callApp(app, `/jobs/${jobId}`, { token: ORG_TOKEN });
...
describe("GET /jobs", () => {
  ...
  it("respects limit and offset parameters", async () => {
    ...
    const { body } = await callApp(app, "/jobs?limit=2&offset=0", { token: ORG_TOKEN });
    const result = body as { jobs: unknown[]; total: number };
