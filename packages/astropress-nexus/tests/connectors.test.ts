...
describe("POST /connectors/cloudways/discover", () => {
  ...
  it("returns empty sites array when API returns no WordPress apps", async () => {
    vi.stubGlobal("fetch", makeSequentialFetchMock([
      { ok: true, status: 200, body: { access_token: "bearer-token-123" } },
      { ok: true, status: 200, body: { apps: [
        { id: "1", label: "Node App", app_fqdn: "node.example.com", application: { type: "nodejs" } },
      ] } },
    ]));
  ...
  it("returns correct siteUrl for WordPress apps", async () => {
    vi.stubGlobal("fetch", makeSequentialFetchMock([
      { ok: true, status: 200, body: { access_token: "bearer-token-123" } },
      { ok: true, status: 200, body: { apps: [
        { id: "1", label: "Blog", app_fqdn: "blog.example.com", application: { type: "wordpress" } },
        { id: "2", label: "Shop", app_fqdn: "shop.example.com", application: { type: "woocommerce" } },
        { id: "3", label: "Node", app_fqdn: "node.example.com", application: { type: "nodejs" } },
      ] } },
    ]));
...
describe("POST /connectors/cpanel/discover", () => {
  ...
  it("maps Softaculous installation JSON to DiscoveredSite array", async () => {
    vi.stubGlobal("fetch", makeSequentialFetchMock([
      { ok: true, status: 200, body: [
        { stype: "wordpress", softurl: "https://blog.example.com", admin_username: "wpadmin" },
        { stype: "joomla", softurl: "https://joomla.example.com", admin_username: "jadmin" },
        { stype: "wordpress", softurl: "https://shop.example.com", admin_username: "shopadmin" },
      ] },
    ]));
    ...
    expect(status).toBe(200);
    const sites = (body as { sites: Array<{ siteUrl: string; metadata: Record<string, unknown> }> }).sites;
    expect(sites).toHaveLength(2);
    expect(sites[0].siteUrl).toBe("https://blog.example.com");
