...
describe("POST /actions/redeploy", () => {
	...
	it("triggers deploy hooks for selected sites and redirects back to dashboard", async () => {
		vi.spyOn(globalThis, "fetch").mockImplementation(
			makeFetchMock((url) =>
				new URL(url).hostname === "deploy.example.com" // audit-ok: test mock routing — uses new URL().hostname (exact equality), not a substring check; codeql[js/incomplete-url-substring-sanitization]
		...
		const res = await app.fetch(
			new Request(`http://localhost/actions/redeploy?token=${ORG_TOKEN}`, {
				method: "POST",
...
describe("POST /actions/refresh", () => {
	...
	it("refreshes selected sites and redirects with a refreshed count", async () => {
		...
		const res = await app.fetch(
			new Request(`http://localhost/actions/refresh?token=${ORG_TOKEN}`, {
				method: "POST",
				headers: { "Content-Type": "application/x-www-form-urlencoded" },
				body: form,
			}),
