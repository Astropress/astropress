...
import type { NexusConfig, FanOutResult, ContentItem, SiteEntry, SiteHealth } from "./types.js";
...
function renderDashboardHtml(input: {
  ...
  const filtered = sites.filter((site) => {
    const haystack = `${site.name} ${site.id} ${site.baseUrl} ${site.description ?? ""}`.toLowerCase();
    return haystack.includes(query.toLowerCase());
  });
  const statusMap = new Map(statuses.map((status) => [status.id, status]));
  const tokenSuffix = authToken ? `?token=${encodeURIComponent(authToken)}` : "";

  const cards = filtered.map((site) => {
      ...
      <article class="site-card">
        <div class="site-card__header">
          <div>
            ...
            <h2><a href="${detailHref}">${escapeHtml(site.name)}</a></h2>
          </div>
    ...
    <body>
      <main>
        ...
        <section class="surface">
          <h2>Recent jobs</h2>
          <ul>
            ${(jobs.jobs.filter((job) => job.siteId === site.id).slice(0, 5).map((job) => `<li><code>${escapeHtml(job.kind)}</code> — ${escapeHtml(job.status)} (${escapeHtml(job.id)})</li>`).join("")) || "<li>No jobs recorded yet.</li>"}
          </ul>
        </section>
      </main>
    </body>
...
export function createNexusApp(options: NexusAppOptions): Hono {
  ...
  app.post("/actions/refresh", async (c) => {
    ...
    const selectedSites = (requestedIds.length > 0 ? requestedIds : registry.getAll().map((site) => site.id))
