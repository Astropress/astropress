...
export async function discoverCPanelWordPressSites(input: CPanelDiscoveryInput): Promise<DiscoveredSite[]> {
  const { host, username, password } = input;

  const basicAuth = btoa(`${username}:${password}`);
  const softaculousUrl =
    `https://${host}:2083/frontend/paper_lantern/softaculous/index.live.php?act=installations&display=JSON`;
