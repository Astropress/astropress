import { appHostToDeployTarget } from "./project-scaffold-env.js";
export function createPackageScripts(appHost) {
    const scripts = {
        dev: "astro dev",
        build: "astro build",
        check: "astro check",
...
function createAstropressConfig(appHost) {
...
function createSecurityWorkflow() {
    return [
        `name: Security`,
        ``,
        `on:`,
...
function createDonatePage(donations, siteUrl) {
    const providers = [];
...
export function createCiFiles(appHost, requiredEnvKeys, donations) {
    const files = {};
...
export function createDeployDoc(appHost, dataServices, supportLevel, requiredEnvKeys) {
