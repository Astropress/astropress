...
function queryStructuredPageRoutes(
	getDb: () => AstropressSqliteDatabaseLike,
): StructuredPageRouteRecord[] {
	const rows = getDb()
		.prepare(SQL_LIST_STRUCTURED)
		.all() as StructuredPageRow[];
	return rows
		.map(mapStructuredPageRow)
