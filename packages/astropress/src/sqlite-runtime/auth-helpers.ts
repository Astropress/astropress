...
export const SQL_FIND_ACTIVE_USER =
	"SELECT id, email, password_hash, CASE WHEN is_admin = 1 THEN 'admin' ELSE 'editor' END AS role, name FROM admin_users WHERE email = ? AND active = 1 LIMIT 1";
...
export const SQL_FIND_LIVE_SESSION =
	"SELECT s.id, s.csrf_token, s.last_active_at, u.email, CASE WHEN u.is_admin = 1 THEN 'admin' ELSE 'editor' END AS role, u.name FROM admin_sessions s JOIN admin_users u ON u.id = s.user_id WHERE s.id = ? AND s.revoked_at IS NULL AND u.active = 1 LIMIT 1";
...
export const SQL_FIND_INVITE =
	"SELECT i.id, i.user_id, i.token_hash, i.expires_at, i.accepted_at, i.created_at, u.email, u.name, CASE WHEN u.is_admin = 1 THEN 'admin' ELSE 'editor' END AS role, u.active FROM user_invites i JOIN admin_users u ON u.id = i.user_id WHERE i.token_hash = ? LIMIT 1";
...
export const SQL_FIND_PW_RESET_USER =
	"SELECT id, email, CASE WHEN is_admin = 1 THEN 'admin' ELSE 'editor' END AS role, name FROM admin_users WHERE email = ? AND active = 1 LIMIT 1";
...
export const SQL_FIND_RESET_TOKEN =
	"SELECT t.id, t.user_id, t.token_hash, t.expires_at, t.consumed_at, t.created_at, u.email, u.name, CASE WHEN u.is_admin = 1 THEN 'admin' ELSE 'editor' END AS role, u.active FROM password_reset_tokens t JOIN admin_users u ON u.id = t.user_id WHERE t.token_hash = ? LIMIT 1";
