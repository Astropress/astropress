...
import type { Actor, ContentRecord, SessionUser } from "../persistence-types";
...
const SQL_INSERT_BASELINE_OVERRIDE =
...
function mapCustomEntry(
	row: ReturnType<typeof queryCustomContentEntries>[number],
): PageRecord {
	return {
		slug: row.slug,
...
function queryContentAssignmentIds(
...
function buildBaselineRevisionParams(
	randomId: () => string,
...
function ensureBaselineRevisionImpl(
	getDb: () => AstropressSqliteDatabaseLike,
	randomId: () => string,
	pageRecord: PageRecord,
) {
...
export function createSqliteContentStore(
	...
	function getAllContentRecords() {
		return [
			...getSeedPageRecords(),
			...getCustomContentEntries().map((row) => mapCustomEntry(row)),
		];
	}
