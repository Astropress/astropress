...
const SQL_READ_RATE_LIMIT = "SELECT count, window_start_ms, window_ms FROM rate_limits WHERE key = ? LIMIT 1";
const SQL_RESET_RATE_LIMIT = `INSERT INTO rate_limits (key, count, window_start_ms, window_ms) VALUES (?, 1, ?, ?) ON CONFLICT(key) DO UPDATE SET count = 1, window_start_ms = excluded.window_start_ms, window_ms = excluded.window_ms`;
const SQL_INC_RATE_LIMIT = "UPDATE rate_limits SET count = count + 1 WHERE key = ?";
const SQL_UPDATE_MEDIA = "UPDATE media_assets SET title = ?, alt_text = ? WHERE id = ? AND deleted_at IS NULL";
const SQL_INSERT_MEDIA = `INSERT INTO media_assets (id, source_url, local_path, r2_key, mime_type, file_size, alt_text, title, uploaded_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
const SQL_GET_MEDIA_FOR_DELETE = "SELECT local_path FROM media_assets WHERE id = ? AND deleted_at IS NULL";
const SQL_SOFT_DELETE_MEDIA = "UPDATE media_assets SET deleted_at = CURRENT_TIMESTAMP WHERE id = ?";
...
export function createSqliteAssetsStore(getDb: () => AstropressSqliteDatabaseLike, now: () => number) {
  ...
  const sqliteRateLimitRepository = createAstropressRateLimitRepository({
    ...
    readRateLimitWindow(key: string) {
      const row = getDb().prepare(SQL_READ_RATE_LIMIT).get(key) as { count: number; window_start_ms: number; window_ms: number } | undefined;
      if (!row) return null;
      return { count: row.count, windowStartMs: row.window_start_ms, windowMs: row.window_ms };
    },
  ...
  const sqliteMediaRepository = createAstropressLocalMediaRepository({
    ...
    getStoredMediaDeletionCandidate(id: string) {
      const row = getDb().prepare(SQL_GET_MEDIA_FOR_DELETE).get(id) as { local_path: string } | undefined;
