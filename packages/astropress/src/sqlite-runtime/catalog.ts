...
export function createSqliteCatalogStore(getDb: () => AstropressSqliteDatabaseLike) {
  ...
  const sqliteTaxonomyRepository = createAstropressTaxonomyRepository({
    ...
    slugifyTerm,
    createTaxonomyTerm({ table, slug, name, description }: { table: "categories" | "tags"; slug: string; name: string; description?: string }) {
      try {
    ...
    updateTaxonomyTerm({ table, id, slug, name, description }: { table: "categories" | "tags"; id: number; slug: string; name: string; description?: string }) {
      try {
        const result = getDb()
          .prepare(
