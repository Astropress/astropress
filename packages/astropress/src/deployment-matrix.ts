...
export type AstropressDeploymentSupportLevel =
	| "supported"
	| "preview"
	| "unsupported";
...
export interface AstropressDeploymentMatrixEntry
	extends AstropressDeploymentProfile {
...
export function getAstropressDeploymentMatrixEntry(
	...
	return (
		deploymentMatrixEntries.find(
			(entry) =>
				entry.appHost === profile.appHost &&
				entry.dataServices === profile.dataServices,
		) ?? null
	);
}
...
export function resolveAstropressDeploymentSupportLevel(
	...
	return (
		getAstropressDeploymentMatrixEntry(profile)?.supportLevel ?? "unsupported"
	);
}
