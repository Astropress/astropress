...
// Data-only sibling of deployment-matrix.ts. Mutating individual notes,
// envKey strings, or supportLevel labels is unkillable by reasonable
// tests — value is in the matrix shape, not the marketing copy.
// Behavior of the accessors is mutation-tested in deployment-matrix.ts
// at 95%+.

import type { AstropressDeploymentMatrixEntry } from "./deployment-matrix";

export const deploymentMatrixEntries: AstropressDeploymentMatrixEntry[] = [
	{
	...
	},
	{
		...
		supportLevel: "preview",
		notes:
			"Railway deployment with Turso (LibSQL) for distributed SQLite data. Paid — usage-based billing.",
		requiredEnvKeys: ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN"],
	},
];
