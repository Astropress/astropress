...
export function parseWixExport(csvText: string): WixParsedBundle {
  ...
  for (const row of rows.slice(1)) {
    ...
    if (!authorSet.has(authorLogin)) authorSet.set(authorLogin, { login: authorLogin, displayName: authorName });

    const categorySlugs: string[] = [];
    for (const cat of splitTermList(col(row, ["categories", "category"]))) {
      const catSlug = slugify(cat);
      categorySlugs.push(catSlug);
      if (!categorySet.has(catSlug)) categorySet.set(catSlug, { kind: "category", slug: catSlug, name: cat });
    }
    ...
    if (coverImageRaw) {
      ...
      if (!mediaSet.has(normalized)) {
        ...
        mediaSet.set(normalized, { id, slug: id, title: title || id, sourceUrl: normalized, filename, mimeType: guessMimeTypeWix(normalized) });
      }
    }
    ...
    for (const imgUrl of scrapeImageUrls(body)) {
      ...
      if (!mediaSet.has(normalized)) {
        const id = `wix-media-${mediaIndex++}`;
        const filename = `${id}${guessExtension(normalized)}`;
        mediaSet.set(normalized, { id, slug: id, title: id, sourceUrl: normalized, filename, mimeType: guessMimeTypeWix(normalized) });
      }
    }
...
async function loadDownloadState(stateFile: string): Promise<DownloadState> {
  try { return JSON.parse(await readFile(stateFile, "utf8")) as DownloadState; }
...
async function downloadWixMedia(
  ...
  for (const asset of assets) {
    ...
    try {
      ...
      await writeFile(path.join(downloadsDir, asset.filename), Buffer.from(await response.arrayBuffer()));
      state.completed.push(asset.id);
      downloadedMedia++;
    } catch (err) {
      state.failed.push({ id: asset.id, sourceUrl: asset.sourceUrl, reason: err instanceof Error ? err.message : String(err) });
    }
  ...
  return {
    async importWix(input: {
      ...
      if (input.applyLocal && input.workspaceRoot) {
        localApply = await applyWixImportToLocalRuntime({ bundle, artifactDir: input.artifactDir, workspaceRoot: input.workspaceRoot, adminDbPath: input.adminDbPath });
      }
