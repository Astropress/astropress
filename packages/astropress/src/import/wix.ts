	...
	for (const asset of assets) {
		...
		try {
			...
			await writeFile(
				...
				Buffer.from(await response.arrayBuffer()), // codeql[js/http-to-file-access] path.basename() strips traversal from the filename; bytes are intentionally written from the HTTP response
			);
