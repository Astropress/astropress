...
export type WixParsedTerm = { kind: "category" | "tag"; slug: string; name: string };
export type WixParsedMediaAsset = { id: string; slug: string; title: string; sourceUrl: string; filename: string; mimeType: string };
export type WixParsedContentRecord = {
  id: string; kind: "post"; slug: string; title: string; body: string; excerpt: string | undefined;
  status: "draft" | "published"; legacyUrl: string; publishedAt: string | undefined;
  authorLogins: string[]; categorySlugs: string[]; tagSlugs: string[];
};
...
export async function fileSizeOrNull(targetPath: string): Promise<number | null> {
  try { return (await stat(targetPath)).size; } catch { return null; }
}

export function resolveAdminDbPath(workspaceRoot: string, adminDbPath?: string): string {
  if (adminDbPath) return path.isAbsolute(adminDbPath) ? adminDbPath : path.join(workspaceRoot, adminDbPath);
  return createDefaultAstropressSqliteSeedToolkit().getDefaultAdminDbPath(workspaceRoot);
}

type WixAdminDb = ReturnType<ReturnType<typeof createDefaultAstropressSqliteSeedToolkit>["openSeedDatabase"]>;
type WixAdminRuntime = ReturnType<typeof createAstropressSqliteAdminRuntime>;

function importWixTerms(
  db: WixAdminDb, bundle: WixParsedBundle,
...
function importWixContentRecords(
  db: WixAdminDb, runtime: WixAdminRuntime, bundle: WixParsedBundle,
  ...
  for (const record of bundle.contentRecords) {
    ...
    const revisionInput = {
      title: record.title, status: record.status, body: record.body,
    ...
    if (existing) {
      const saved = runtime.sqliteAdminStore.content.saveContentState(record.slug, revisionInput, WIX_IMPORT_ACTOR);
...
async function importWixMedia(
  db: WixAdminDb, bundle: WixParsedBundle, artifactDir: string | undefined,
) {
  ...
  for (const asset of bundle.mediaAssets) {
    ...
    upsertMedia.run(
      ...
      "", asset.title, WIX_IMPORT_ACTOR.email,
    );
  }
}
...
export async function applyWixImportToLocalRuntime(input: {
  ...
  try {
    ...
    importWixContentRecords(db, runtime, input.bundle, authorIdsByLogin, categoryIdsBySlug, tagIdsBySlug);
    await importWixMedia(db, input.bundle, input.artifactDir);

    db.exec("COMMIT");
    return { appliedRecords: input.bundle.contentRecords.length, appliedMedia: input.bundle.mediaAssets.length, appliedAuthors: input.bundle.authors.length, adminDbPath: resolvedDbPath };
  } catch (error) {
    db.exec("ROLLBACK");
    throw error;
  } finally {
