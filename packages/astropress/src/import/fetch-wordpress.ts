...
async function attemptWordPressExport(opts: WordPressFetchOptions & { headless: boolean }): Promise<WordPressFetchResult> {
  ...
  try {
    ...
    try {
      loginResponse = await page.goto(loginUrl, { timeout: timeoutMs, waitUntil: "domcontentloaded" }) as typeof loginResponse;
    } catch (err) {
...
export async function fetchWordPressExport(opts: WordPressFetchOptions): Promise<WordPressFetchResult> {
