...
function classifyNetworkError(err, hostname) {
  ...
  if (msg.includes("ERR_CERT_") || msg.includes("SSL") || msg.includes("certificate")) {
    return new SiteNotReachableError(hostname, "SSL/TLS error — try using http:// instead of https://");
  }
...
async function attemptWordPressExport(opts) {
  ...
  try {
    ...
    let loginResponse;
    try {
      loginResponse = await page.goto(loginUrl, { timeout: timeoutMs, waitUntil: "domcontentloaded" });
    } catch (err) {
    ...
    // 4. Check for 2FA (can appear on login page OR wp-admin via plugin redirect)
    const has2FA = await page.locator("[name='authcode'], [name='two-factor-totp-authcode'], [name='mfa_token']").count();
    if (has2FA > 0) {
    ...
    await page.goto(exportUrl, { timeout: timeoutMs, waitUntil: "domcontentloaded" });
...
export async function fetchWordPressExport(opts) {
