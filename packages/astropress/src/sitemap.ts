...
export async function generateAstropressSitemap(
  ...
  for (const post of posts) {
    entries.push({
      ...
      lastmod: typeof post.metadata?.updatedAt === "string" ? post.metadata.updatedAt.slice(0, 10) : undefined,
      changefreq: "monthly",
      priority: 0.7,
    });
  }
  ...
  for (const page of pages) {
    entries.push({
      ...
      lastmod: typeof page.metadata?.updatedAt === "string" ? page.metadata.updatedAt.slice(0, 10) : undefined,
      changefreq: "monthly",
      priority: 0.8,
    });
  }
