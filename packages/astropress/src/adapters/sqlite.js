...
import { pbkdf2Sync as pbkdf2Sync2, randomBytes as randomBytes2 } from "node:crypto";
import { existsSync as existsSync2, mkdirSync as mkdirSync2, readFileSync as readFileSync2, rmSync } from "node:fs";
...
function defaultAdminDbPath(workspaceRoot2 = process.cwd()) {
  return path3.join(workspaceRoot2, ".data", "admin.sqlite");
}
...
function readAstropressSqliteSchemaSql() {
  return readFileSync2(resolveAstropressSqliteSchemaPath(), "utf8");
}
function hashPasswordSync2(password, iterations = 1e5) {
  const salt = randomBytes2(32);
  const derived = pbkdf2Sync2(password, salt, iterations, 64, "sha256");
  return `${iterations}$${salt.toString("base64")}$${derived.toString("base64")}`;
}
...
function createAstropressSqliteSeedToolkit(options) {
  ...
  function getDefaultAdminDbPath(workspaceRoot2 = process.cwd()) {
    return options.getDefaultAdminDbPath?.(workspaceRoot2) ?? defaultAdminDbPath(workspaceRoot2);
  }
  ...
  function seedBootstrapUsers(db) {
    ...
    for (const user of options.loadBootstrapUsers()) {
      const result = upsert.run(user.email.toLowerCase(), hashPasswordSync2(user.password), user.role, user.name);
      count += result.changes ?? 1;
    }
    return count;
  }
  function seedMediaAssets(db, workspaceRoot2) {
    ...
    `);
    let count = 0;
    for (const asset of options.loadMediaSeeds(workspaceRoot2)) {
  ...
  function seedDatabase(seedOptions = {}) {
    const workspaceRoot2 = seedOptions.workspaceRoot ?? process.cwd();
    const dbPath = seedOptions.dbPath ?? getDefaultAdminDbPath(workspaceRoot2);
    const ownsConnection = !seedOptions.db;
    if (seedOptions.reset && dbPath !== ":memory:") {
      rmSync(dbPath, { force: true });
    }
    if (ownsConnection && dbPath !== ":memory:") {
      mkdirSync2(path3.dirname(dbPath), { recursive: true });
    }
    ...
    try {
      ...
      return {
        ...
        mediaAssets: seedMediaAssets(db, workspaceRoot2),
  ...
  function buildSeedImportSql(workspaceRoot2 = process.cwd()) {
    ...
    try {
      ...
      try {
        const tableSql = buildSeedImportStatements(workspaceRoot2, db).map((statement) => statement.sql);
...
function createAstropressSqliteAdapter(options = {}) {
  ...
  const dbPath = options.dbPath ?? seedToolkit.getDefaultAdminDbPath(workspaceRoot2);
  let seeded = false;
  let database = null;
  function ensureDatabase() {
    if (!seeded) {
      seedToolkit.seedDatabase({ dbPath, workspaceRoot: workspaceRoot2 });
      seeded = true;
    }
    if (!database) {
  ...
  return {
    ...
    content: {
      async list(kind) {
        ...
        if (!kind || kind === "translation") {
          const rows = ensureDatabase().prepare("SELECT route, state, updated_at, updated_by FROM translation_overrides ORDER BY route ASC").all();
      ...
      async save(record) {
        ...
        if (record.kind === "page" || record.kind === "post") {
          ...
          if (existing) {
            const result2 = runtime.sqliteAdminStore.content.saveContentState(slug, {
              ...
              ogDescription: typeof record.metadata?.ogDescription === "string" ? record.metadata.ogDescription : undefined,
              ogImage: typeof record.metadata?.ogImage === "string" ? record.metadata.ogImage : undefined,
              canonicalUrlOverride: typeof record.metadata?.canonicalUrlOverride === "string" ? record.metadata.canonicalUrlOverride : undefined,
              robotsDirective: typeof record.metadata?.robotsDirective === "string" ? record.metadata.robotsDirective : undefined
            }, actor);
            if (!result2.ok) {
              throw new Error(result2.error);
            }
            return toContentStoreRecord(result2.state);
          }
      ...
      async put(asset) {
        ...
        ensureDatabase().prepare(`
              ...
              ON CONFLICT(id) DO UPDATE SET
                ...
                title = excluded.title,
                deleted_at = NULL
            `).run(asset.id, asset.publicUrl ?? null, asset.publicUrl ?? `/media/${asset.filename}`, asset.mimeType, asset.bytes?.byteLength ?? null, String(asset.metadata?.altText ?? ""), String(asset.metadata?.title ?? asset.filename), now, actor.email);
        return asset;
      },
      async get(id) {
    ...
    revisions: {
      async list(recordId) {
        return (runtime.sqliteAdminStore.content.getContentRevisions(recordId) ?? []).map((revision) => ({
          id: revision.id,
      ...
      async append(revision) {
        ...
        ensureDatabase().prepare(`
            ...
            `).run(revision.id, revision.recordId, String(snapshot.title ?? revision.recordId), snapshot.status === "archived" ? "archived" : snapshot.status === "draft" ? "draft" : "published", snapshot.scheduledAt ?? null, snapshot.body ?? null, String(snapshot.seoTitle ?? snapshot.title ?? revision.recordId), String(snapshot.metaDescription ?? snapshot.title ?? revision.recordId), snapshot.excerpt ?? null, snapshot.ogTitle ?? null, snapshot.ogDescription ?? null, snapshot.ogImage ?? null, JSON.stringify(snapshot.authorIds ?? []), JSON.stringify(snapshot.categoryIds ?? []), JSON.stringify(snapshot.tagIds ?? []), snapshot.canonicalUrlOverride ?? null, snapshot.robotsDirective ?? null, revision.summary ?? null, revision.createdAt, revision.actorId ?? actor.email);
        return revision;
      }
    }
  };
