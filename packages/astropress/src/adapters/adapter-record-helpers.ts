...
export function toRedirectRecord(rule: { sourcePath: string; targetPath: string; statusCode: 301 | 302 }) {
  return {
    id: rule.sourcePath, kind: "redirect" as const, slug: rule.sourcePath,
    status: "published" as const, title: rule.sourcePath,
    metadata: { targetPath: rule.targetPath, statusCode: rule.statusCode },
  };
}

export function toTranslationRecord(route: string, state: string, updatedAt: string, updatedBy: string) {
  return {
    id: route, kind: "translation" as const, slug: route,
    status: state === "published" ? "published" : "draft",
    title: route, metadata: { state, updatedAt, updatedBy },
  };
}
...
function buildRevisionBindParams(revision: RevisionRecord, actorEmail: string) {
  ...
  return [
    revision.id, revision.recordId, title, status,
    snapshot.scheduledAt ?? null, snapshot.body ?? null,
    seoTitle, metaDescription, snapshot.excerpt ?? null,
    snapshot.ogTitle ?? null, snapshot.ogDescription ?? null, snapshot.ogImage ?? null,
    JSON.stringify(snapshot.authorIds ?? []), JSON.stringify(snapshot.categoryIds ?? []), JSON.stringify(snapshot.tagIds ?? []),
    snapshot.canonicalUrlOverride ?? null, snapshot.robotsDirective ?? null,
    revision.summary ?? null, revision.createdAt, revision.actorId ?? actorEmail,
  ];
}

export async function saveD1Revision(db: D1DatabaseLike, revision: RevisionRecord, actorEmail: string) {
  const params = buildRevisionBindParams(revision, actorEmail);
  await db.prepare(SQL_D1_INSERT_REVISION).bind(...params).run();
}
