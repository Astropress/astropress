...
function deriveLegacyProvider(dataServices) {
...
function defaultAdminDbPath(provider) {
...
function defaultServiceOrigin(dataServices) {
  switch (dataServices) {
    case "supabase":
...
function resolveProfile(input = "sqlite") {
  ...
  const dataServices = input.dataServices ?? (input.legacyProvider === "supabase"
    ...
    : input.legacyProvider === "runway"
      ...
      : "none");
  return {
    appHost: input.appHost ?? (dataServices === "cloudflare"
      ? "cloudflare-pages"
...
function baseLocalEnv(provider, appHost, dataServices) {
  return {
...
function baseEnvExample(provider, appHost, dataServices) {
  return {
    ASTROPRESS_APP_HOST: appHost,
    ASTROPRESS_CONTENT_SERVICES: dataServices,
    ADMIN_DB_PATH: defaultAdminDbPath(provider),
...
function buildDataServiceExample(dataServices) {
...
function createPackageScripts(appHost) {
  const scripts = {
    dev: "astro dev",
    build: "astro build",
    check: "astro check",
...
function gitHubActionsDeployWorkflow(appHost, requiredEnvKeys) {
  const install = [
  ...
  const envComment = requiredEnvKeys.length > 0
...
function gitLabPagesWorkflow() {
  return `image: oven/bun:1\n\npages:\n  stage: deploy\n  script:\n    - bun install\n    - bun run doctor:strict\n    - bun run build\n    - mv dist public\n  artifacts:\n    paths:\n      - public\n  only:\n    - main\n`;
}

function createCiFiles(appHost, requiredEnvKeys) {
...
function createDeployDoc(appHost, dataServices, supportLevel, requiredEnvKeys) {
