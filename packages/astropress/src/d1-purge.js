export function createD1PurgeOps(db) {
  return {
    async purgeUserData(email, options = {}) {
      ...
      if (userRow) {
        const result = await db.prepare("DELETE FROM admin_sessions WHERE user_id = ?").bind(userRow.id).run();
      ...
      const commentsResult = await db.prepare("DELETE FROM comments WHERE email = ?").bind(email).run();
