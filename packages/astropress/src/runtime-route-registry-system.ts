...
async function persistSystemRouteChanges(
  ...
  renderStrategy: RuntimeSystemRouteRecord["renderStrategy"],
  normalizedPath: string,
  validated: { title: string; summary: string | null; bodyHtml: string | null; settingsJson: string | null },
  settings: Record<string, unknown> | null | undefined,
  ...
  await db
    ...
    .bind(validated.title, validated.summary, validated.bodyHtml, validated.settingsJson, validated.title, metaDescription, actor.email, routeId)
    .run();
...
export async function saveRuntimeSystemRoute(
  ...
  return persistSystemRouteChanges(db, route.id, route.render_strategy, normalizedPath, validated, input.settings, input.revisionNote, actor, locals);
}
