...
import { ensureLocalUploadsDir, getLocalUploadsDir, guessImageMimeType } from "./local-image-storage";
...
export function buildLocalMediaDescriptor(input: {
  ...
  if (!allowedExtensions.has(extension)) {
    return { ok: false as const, error: "File type is not allowed. Upload JPEG, PNG, WebP, GIF, AVIF, or SVG images." };
  }
