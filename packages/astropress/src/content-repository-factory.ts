...
export interface AstropressContentRepositoryInput {
  ...
  insertReviewedRevision(
    slug: string,
    revision: Omit<ContentRevision, "id" | "slug" | "source" | "createdAt" | "createdBy"> & { revisionNote?: string },
    actor: Actor,
  ): void;
...
function normalizeAssignments(values?: number[]): number[] {
  return [...new Set((values ?? []).filter((entry) => Number.isInteger(entry) && entry > 0))];
}

function mapContentState(
