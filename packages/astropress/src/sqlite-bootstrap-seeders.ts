...
export function seedBootstrapUsers(options: AstropressSqliteSeedToolkitOptions, db: SqliteDatabaseLike) {
  ...
  for (const user of options.loadBootstrapUsers()) {
    const result = upsert.run(user.email.toLowerCase(), hashPasswordSync(user.password), user.role, user.name) as { changes?: number };
    count += result.changes ?? 1;
  }
  return count;
}

export function seedMediaAssets(options: AstropressSqliteSeedToolkitOptions, db: SqliteDatabaseLike, workspaceRoot: string) {
