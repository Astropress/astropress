...
import type { AuditEvent, CommentRecord, RedirectRule, ContentRecord as PersistedContentRecord } from "./persistence-types";
...
type DashboardDeps = {
  ...
  getRuntimeTranslationState: (route: string, fallback: string, locals: APIContext["locals"]) => Promise<string>;
...
export type AdminDashboardModel = {
  ...
  recentActivity: Array<{ title: string; updatedAt?: string; editHref: string; kind: string }>;
  translationNeedsAttention: number;
  seoNeedsAttention: number;
  archiveRoutes: ArchiveRoute[];
  supportSurfaceLinks: Array<{ label: string; href: string; count: number; helper: string }>;
};
...
export async function buildAdminDashboardModel(
  ...
  const [auditEvents, comments, redirectRules, routePages, contentStates, systemRoutes] = await Promise.all([
  ...
  const reviewPosts = posts.filter((record) => (record.status ?? "published") === "review" || (record.status ?? "published") === "draft");
  const scheduledPosts = posts
    .filter((record) => record.scheduledAt && Number.isFinite(Date.parse(record.scheduledAt)) && Date.parse(record.scheduledAt) > now)
  ...
  const translationEntries = role === "admin"
    ? await settledValue(
        Promise.all(
          translationStatus.map(async (entry) => ({
            route: entry.route,
            state: await deps.getRuntimeTranslationState(entry.route, entry.translationState, locals),
          })),
        ),
  ...
  const translationNeedsAttention = translationEntries.filter((entry) => entry.state !== "published").length;
  ...
  const archiveRoutes: ArchiveRoute[] = role === "admin"
    ? await settledValue(
        Promise.all([
