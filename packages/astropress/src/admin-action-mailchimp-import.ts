...
export async function runMailchimpImport(
  ...
  if (
    ...
    return {
      ...
      imported: 0,
      skipped: 0,
      error: "Listmonk is not configured. Set NEWSLETTER_DELIVERY, LISTMONK_API_URL, LISTMONK_API_USERNAME, LISTMONK_API_PASSWORD, and LISTMONK_LIST_ID.",
    };
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { ok: false, imported: 0, skipped: 0, error: `Listmonk returned ${res.status}. Check your credentials.` };
    }
    ...
    return { ok: false, imported: 0, skipped: 0, error: "Network error reaching Listmonk. Check LISTMONK_API_URL." };
  }
}
