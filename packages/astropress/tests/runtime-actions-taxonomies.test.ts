...
beforeEach(() => {
  ...
  db.prepare("INSERT INTO authors (name, slug, bio) VALUES (?, ?, ?)").run("Existing Author", "existing-author", "Bio");
  db.prepare("INSERT INTO categories (name, slug) VALUES (?, ?)").run("Existing Cat", "existing-cat");
...
describe("createRuntimeAuthor", () => {
  it("creates an author", async () => {
    ...
    const row = db.prepare("SELECT name FROM authors WHERE slug = 'new-author'").get() as { name: string } | undefined;
...
describe("updateRuntimeAuthor", () => {
  it("updates name and bio", async () => {
    const { lastInsertRowid: id } = db.prepare("INSERT INTO authors (name, slug) VALUES (?, ?)").run("Old", "old-author");
    const result = await updateRuntimeAuthor({ id: Number(id), name: "New Name", bio: "New bio" }, actor, locals);
    expect(result).toMatchObject({ ok: true });
    const row = db.prepare("SELECT name, bio FROM authors WHERE id = ?").get(Number(id)) as { name: string; bio: string };
...
describe("deleteRuntimeAuthor", () => {
  it("soft-deletes an author", async () => {
    const { lastInsertRowid: id } = db.prepare("INSERT INTO authors (name, slug) VALUES (?, ?)").run("To Delete", "to-delete");
    const result = await deleteRuntimeAuthor(Number(id), actor, locals);
    expect(result).toMatchObject({ ok: true });
    const row = db.prepare("SELECT deleted_at FROM authors WHERE id = ?").get(Number(id)) as { deleted_at: string | null };
...
describe("updateRuntimeCategory", () => {
  it("updates a category", async () => {
    const { lastInsertRowid: id } = db.prepare("INSERT INTO categories (name, slug) VALUES (?, ?)").run("Old Cat", "old-cat");
...
describe("deleteRuntimeCategory", () => {
  it("soft-deletes a category", async () => {
    const { lastInsertRowid: id } = db.prepare("INSERT INTO categories (name, slug) VALUES (?, ?)").run("Delete Cat", "delete-cat");
    await deleteRuntimeCategory(Number(id), actor, locals);
    const row = db.prepare("SELECT deleted_at FROM categories WHERE id = ?").get(Number(id)) as { deleted_at: string | null };
    expect(row.deleted_at).not.toBeNull();
  });
});
...
describe("deleteRuntimeTag", () => {
  it("soft-deletes a tag", async () => {
    const { lastInsertRowid: id } = db.prepare("INSERT INTO tags (name, slug) VALUES (?, ?)").run("Delete Tag", "delete-tag");
    await deleteRuntimeTag(Number(id), actor, locals);
    const row = db.prepare("SELECT deleted_at FROM tags WHERE id = ?").get(Number(id)) as { deleted_at: string | null };
    expect(row.deleted_at).not.toBeNull();
  });
});
