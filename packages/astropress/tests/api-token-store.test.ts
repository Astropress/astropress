...
describe("ApiTokenStore SQLite implementation", () => {
  it("create: stores hashed token and returns raw token once", async () => {
    ...
    const { record, rawToken } = await store.create({ label: "test", scopes: ["content:read"] });
  ...
  it("verify: revoked token returns { valid: false }", async () => {
    const db = makeDb();
    const store = createApiTokenStore(db);
    const { record, rawToken } = await store.create({ label: "short-lived", scopes: ["settings:read"] });
  ...
  it("revoke: sets revokedAt timestamp", async () => {
    const db = makeDb();
    const store = createApiTokenStore(db);
    const { record } = await store.create({ label: "revoke-me", scopes: ["content:write"] });
  ...
  it("list: revoked tokens still appear in list (for admin display)", async () => {
    const db = makeDb();
    const store = createApiTokenStore(db);
    const { record } = await store.create({ label: "revocable", scopes: ["content:read"] });
    await store.revoke(record.id);
