...
import { registerCms, peekCmsConfig, dispatchPluginContentEvent, dispatchPluginMediaEvent } from "../src/config.js";
...
describe("AstropressPlugin API", () => {
  it("dispatchPluginContentEvent calls onContentSave on registered plugins", async () => {
    ...
    const plugin: AstropressPlugin = {
      name: "test-plugin",
      onContentSave: ({ slug }) => { events.push(`save:${slug}`); },
    };
    registerCms(makeMinimalConfig([plugin]));

    await dispatchPluginContentEvent("onContentSave", { slug: "my-post", kind: "post", status: "draft", actor: "admin@example.com" });
  ...
  it("dispatchPluginContentEvent calls onContentPublish only for published status", async () => {
    ...
    const plugin: AstropressPlugin = {
      name: "publish-plugin",
      onContentPublish: ({ slug }) => { events.push(`publish:${slug}`); },
    };
    registerCms(makeMinimalConfig([plugin]));

    await dispatchPluginContentEvent("onContentPublish", { slug: "my-post", kind: "post", status: "published", actor: "admin@example.com" });
    await dispatchPluginContentEvent("onContentSave", { slug: "draft-post", kind: "post", status: "draft", actor: "admin@example.com" });
  ...
  it("dispatchPluginContentEvent catches errors thrown by plugin hooks", async () => {
    const errorPlugin: AstropressPlugin = {
      name: "broken-plugin",
      onContentSave: () => { throw new Error("plugin error"); },
    };
    registerCms(makeMinimalConfig([errorPlugin]));

    // Should not throw
    await expect(
      dispatchPluginContentEvent("onContentSave", { slug: "post", kind: "post", status: "draft", actor: "admin@example.com" }),
  ...
  it("dispatchPluginMediaEvent calls onMediaUpload on registered plugins", async () => {
    ...
    const plugin: AstropressPlugin = {
      name: "media-plugin",
      onMediaUpload: ({ id, mimeType }) => { uploads.push(`${id}:${mimeType}`); },
    };
    registerCms(makeMinimalConfig([plugin]));

    await dispatchPluginMediaEvent({ id: "asset-1", filename: "photo.jpg", mimeType: "image/jpeg", size: 1024, actor: "admin@example.com" });
  ...
  it("dispatchPluginMediaEvent catches errors thrown by onMediaUpload hooks", async () => {
    const brokenPlugin: AstropressPlugin = {
      ...
      onMediaUpload: () => { throw new Error("media plugin error"); },
    };
    registerCms(makeMinimalConfig([brokenPlugin]));

    await expect(
      dispatchPluginMediaEvent({ id: "a", filename: "f.jpg", mimeType: "image/jpeg", size: 0, actor: "x@y.com" }),
    ).resolves.toBeUndefined();
  });
