...
describe("ZTA P1: every request must prove identity", () => {
	...
	it("admin pages read Astro.locals.adminUser — auth is resolved in middleware, not ad-hoc", () => {
		...
		for (const file of pageFiles) {
			...
			expect(
				hasAuthCheck,
				`${rel}: admin page must read session user from locals or redirect to login`,
			).toBe(true);
		}
	});
...
describe("ZTA P2: least privilege", () => {
	...
	it("API token scope is validated per-request, not cached", () => {
		...
		expect(
			apiMiddleware,
			"API middleware must not cache token scope results at module level",
		).not.toMatch(/const\s+scopeCache\s*=|const\s+tokenCache\s*=/);
	});
...
describe("ZTA P3: assume breach — audit trail and trace propagation", () => {
	it("security middleware attaches X-Request-Id to every response", () => {
		...
		expect(
			middleware,
			"X-Request-Id must be a fresh UUID per request, not a static value",
	...
	it("audit_events table is write-only from application code — no targeted DELETE in audit log code", () => {
		...
		expect(
			...
			/DELETE\s+FROM\s+audit_events\s+WHERE\s+(id|user_email|action|resource_type|resource_id)\s*[=<>]/i,
		);
	});
