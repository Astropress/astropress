...
function seedUser(db: DatabaseSync, email: string, name = "Test User") {
  ...
  const row = db.prepare("SELECT id FROM admin_users WHERE email = ? LIMIT 1").get(email) as { id: number };
...
describe("createSqlitePurgeOps — user data purge (GDPR)", () => {
  ...
  it("anonymises audit events", () => {
    ...
    const row = db.prepare("SELECT user_email FROM audit_events LIMIT 1").get() as { user_email: string };
  ...
  it("deletes comments by the user", () => {
    ...
    const remaining = db.prepare("SELECT COUNT(*) as cnt FROM comments WHERE email = 'alice@test.com'").get() as { cnt: number };
  ...
  it("suspends admin user by default", () => {
    ...
    const row = db.prepare("SELECT active FROM admin_users WHERE email = 'alice@test.com'").get() as { active: number };
    expect(row.active).toBe(0);
  });

  it("deletes admin user when deleteAccount: true", () => {
    seedUser(db, "alice@test.com");
    const result = purge.purgeUserData("alice@test.com", { deleteAccount: true });
    expect(result.adminUserAction).toBe("deleted");
    const row = db.prepare("SELECT COUNT(*) as cnt FROM admin_users WHERE email = 'alice@test.com'").get() as { cnt: number };
