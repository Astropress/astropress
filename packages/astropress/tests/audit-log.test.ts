...
describe("audit log", () => {
  ...
  it("stores and retrieves structured details JSON", () => {
    ...
    expect(events[0].details).toEqual({ role: "editor", invitedBy: "admin@example.com" });
  });
  ...
  it("prunes audit events older than auditRetentionDays on write", () => {
    ...
    const before = (freshDb.prepare("SELECT COUNT(*) as n FROM audit_events").get() as { n: number }).n;
    ...
    const after = (freshDb.prepare("SELECT COUNT(*) as n FROM audit_events").get() as { n: number }).n;
    // Old event should be pruned; new event should remain
    expect(after).toBe(1);
    const remaining = freshDb.prepare("SELECT resource_id FROM audit_events").get() as { resource_id: string };
