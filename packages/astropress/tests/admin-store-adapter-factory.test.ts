  ...
  it("creates an AdminStoreAdapter with the provided backend and modules", async () => {
    const adapter = createAstropressAdminStoreAdapter("sqlite", {
      auth: {
        ...
        consumePasswordResetToken: vi.fn(async () => ({ ok: false, error: "nope" })),
      ...
      submissions: {
        submitContact: vi.fn(async () => ({ ok: true, submission: { id: "1", name: "A", email: "a@example.com", message: "x", submittedAt: "now" } })),
        getContactSubmissions: vi.fn(async () => []),
      },
      translations: {
        updateTranslationState: vi.fn(async () => ({ ok: false, error: "nope" })),
