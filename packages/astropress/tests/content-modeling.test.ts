...
describe("validateContentFields — content-ref type", () => {
  ...
  it("fails required content-ref when empty", () => {
    const requiredType: ContentTypeDefinition = {
      key: "article",
      label: "Article",
      fields: [{ name: "relatedPost", label: "Related Post", type: "content-ref", required: true }],
    };
...
describe("validateContentFields — repeater type", () => {
  ...
  it("passes when value is a valid array of objects", () => {
    expect(validateContentFields(contentType, { speakers: [{ name: "Alice", bio: "Engineer" }] })).toBeNull();
  });
  ...
  it("fails when value is not an array", () => {
    const error = validateContentFields(contentType, { speakers: "not-an-array" });
...
describe("SQLite upsertContentOverride — metadata persistence", () => {
  ...
  it("persists metadata through upsert and returns it on read", () => {
    ...
    const actor = { email: "editor@test.com", role: "admin" as const, name: "Editor" };
  ...
  it("stores null metadata when not provided", () => {
    ...
    store.saveContentState(
      "no-meta-post",
      { title: "No Meta Post", status: "draft", seoTitle: "No Meta", metaDescription: "desc" },
      actor,
    );
  ...
  it("updates metadata on subsequent upsert", () => {
    ...
    store.saveContentState(
      "update-meta-post",
      { title: "Post", status: "draft", seoTitle: "Post", metaDescription: "desc", metadata: { version: 1 } },
      actor,
    );
    store.saveContentState(
      "update-meta-post",
      { title: "Post", status: "draft", seoTitle: "Post", metaDescription: "desc", metadata: { version: 2 } },
      actor,
    );
