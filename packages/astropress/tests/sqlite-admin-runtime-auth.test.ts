...
describe("factory option defaults", () => {
  it("creates runtime with default sessionTtlMs, now, and randomId", () => {
    const rt = createAstropressSqliteAdminRuntime({ getDatabase: () => fixture.db });
...
describe("auth", () => {
  ...
  it("returns null for malformed password hash (verifyPasswordSync malformed-hash branch)", async () => {
    expect(await fixture.runtime.authenticatePersistedAdminUser("malformed@test.local", "any-password")).toBeNull();
  });

  it("returns null for wrong password", async () => {
    expect(await fixture.runtime.authenticatePersistedAdminUser("admin@test.local", "wrong-password")).toBeNull();
  });

  it("returns user for correct password", async () => {
    const user = await fixture.runtime.authenticatePersistedAdminUser("admin@test.local", "correct-password");
  ...
  it("session lifecycle: create → get → getCsrfToken → revoke", async () => {
    const user = await fixture.runtime.authenticatePersistedAdminUser("admin@test.local", "correct-password");
    const sessionToken = fixture.store.auth.createSession(user!, { ipAddress: "127.0.0.1" });
  ...
  it("createPasswordResetToken: empty email returns ok:false", () => {
    expect(fixture.store.auth.createPasswordResetToken("", fixture.actor)).toMatchObject({ ok: false });
  });
  ...
  it("consumeInviteToken: short password returns error", () => {
    expect(fixture.store.auth.consumeInviteToken("any-token", "short")).toMatchObject({ ok: false });
  });

  it("consumeInviteToken: invalid token returns error", () => {
    expect(fixture.store.auth.consumeInviteToken("not-a-real-token", "long-enough-password")).toMatchObject({ ok: false });
  });

  it("consumePasswordResetToken: short password returns error", () => {
    expect(fixture.store.auth.consumePasswordResetToken("any-token", "short")).toMatchObject({ ok: false });
  });

  it("consumePasswordResetToken: invalid token returns error", () => {
    expect(fixture.store.auth.consumePasswordResetToken("not-a-real-token", "long-enough-password")).toMatchObject({ ok: false });
  });
  ...
  it("full password reset flow: issue, get request, consume", () => {
    ...
    const consumed = fixture.store.auth.consumePasswordResetToken(rawToken, "new-long-password-123");
  ...
  it("full invite flow: create invite, get request, consume", async () => {
    ...
    const consumed = fixture.store.auth.consumeInviteToken(inviteRawToken, "my-new-secure-password");
    expect(consumed.ok).toBe(true);
