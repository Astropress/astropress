...
describe("createAstropressAuthRepository", () => {
  it("authenticates a persisted admin user", async () => {
    ...
    await expect(repository.authenticatePersistedAdminUser("admin@example.com", "correct horse battery staple")).resolves.toEqual({
      email: "admin@example.com",
      role: "admin",
      name: "Admin",
    });
  ...
  it("creates and reads a live session", () => {
    ...
    expect(repository.createSession({ email: "admin@example.com", role: "admin", name: "Admin" })).toBe("session-1");
  ...
  it("accepts a valid invite token", () => {
    ...
    expect(updateAdminUserPassword).toHaveBeenCalledWith(5, "password:correct horse battery staple");
  ...
  it("creates and consumes a password reset token", () => {
    ...
    expect(repository.consumePasswordResetToken("raw-reset-token", "correct horse battery staple")).toEqual({
      ok: true,
      user: {
        email: "admin@example.com",
        role: "admin",
