...
describe("readAstropressNeonHostedConfig", () => {
  it("reads DATABASE_URL from env", () => {
    ...
    expect(config.databaseUrl).toBe("postgresql://user:pass@ep-example.us-east-1.aws.neon.tech/neondb");
  ...
  it("throws when DATABASE_URL is not a postgres URL", () => {
    expect(() =>
      readAstropressNeonHostedConfig({ DATABASE_URL: "mysql://user:pass@host/db" }),
    ).toThrow(/postgres/);
  });

  it("accepts both postgres:// and postgresql:// schemes", () => {
    expect(() =>
      readAstropressNeonHostedConfig({ DATABASE_URL: "postgres://u:p@host/db" }),
    ).not.toThrow();
    expect(() =>
      readAstropressNeonHostedConfig({ DATABASE_URL: "postgresql://u:p@host/db" }),
    ).not.toThrow();
  });
