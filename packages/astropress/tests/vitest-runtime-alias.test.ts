...
describe("vitest runtime alias helpers", () => {
  it("creates the local module replacement and external source rewrite plugins", () => {
    const [replacePlugin, rewritePlugin] = createAstropressVitestLocalRuntimePlugins(
  ...
  it("does not rewrite unrelated imports", () => {
    ...
    expect(rewritePlugin.resolveId("./other-module", "/tmp/site/node_modules/astropress/src/runtime-page-store.ts")).toBe(
      undefined,
    );
    expect(rewritePlugin.resolveId("./local-runtime-modules", "/tmp/site/src/app.ts")).toBe(undefined);
  });
});
