...
describe("buildLocalMediaDescriptor", () => {
  it("returns a valid descriptor for a PNG upload", () => {
    const result = buildLocalMediaDescriptor({ filename: "photo.png", bytes: validPng });
  ...
  it("falls back to sanitised basename as title", () => {
    const result = buildLocalMediaDescriptor({ filename: "my awesome photo.png", bytes: validPng });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it("rejects zero-byte file", () => {
    const result = buildLocalMediaDescriptor({ filename: "empty.png", bytes: new Uint8Array(0) });
    expect(result).toMatchObject({ ok: false });
  });

  it("rejects file exceeding 10 MB", () => {
    const result = buildLocalMediaDescriptor({ filename: "huge.png", bytes: oversizedFile });
    expect(result).toMatchObject({ ok: false });
    expect((result as { error: string }).error).toContain("10 MB");
  });

  it("rejects disallowed extension (.php)", () => {
    const result = buildLocalMediaDescriptor({ filename: "malicious.php", bytes: validPng });
    expect(result).toMatchObject({ ok: false });
    expect((result as { error: string }).error).toContain("not allowed");
  });

  it("rejects disallowed extension (.exe)", () => {
    const result = buildLocalMediaDescriptor({ filename: "evil.exe", bytes: validPng });
    expect(result).toMatchObject({ ok: false });
  });

  it("accepts all allowed extensions", () => {
    const allowed = ["photo.jpg", "photo.jpeg", "photo.png", "photo.webp", "photo.gif", "photo.avif", "photo.svg"];
  ...
  it("generates a unique id on each call", () => {
    const r1 = buildLocalMediaDescriptor({ filename: "a.png", bytes: validPng });
    const r2 = buildLocalMediaDescriptor({ filename: "b.png", bytes: validPng });
...
describe("createLocalMediaUpload", () => {
  it("writes the file to disk and returns descriptor", () => {
    const result = createLocalMediaUpload({ filename: "test.png", bytes: validPng });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it("propagates validation errors without writing to disk", () => {
    const result = createLocalMediaUpload({ filename: "bad.exe", bytes: validPng });
    expect(result).toMatchObject({ ok: false });
  });
});
...
describe("deleteLocalMediaUpload", () => {
  it("deletes a file written to the uploads dir", () => {
    const result = createLocalMediaUpload({ filename: "del.png", bytes: validPng });
    if (!result.ok) return;
    expect(existsSync(result.asset.diskPath)).toBe(true);
