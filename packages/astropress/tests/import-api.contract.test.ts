...
const MOCK_REPORT: AstropressWordPressImportReport = {
  ...
  plan: {
    ...
    entityCounts: { posts: 2, pages: 1, attachments: 1, redirects: 2, comments: 0, users: 1, categories: 0, tags: 0 },
    reviewRequired: false,
    manualTasks: [],
  },
  inventory: {
...
describe("POST /ap-api/v1/import/wordpress — auth and scope", () => {
  ...
  it("returns 403 when token lacks import:write scope", async () => {
    const res = await POST(ctx(req({ token: contentReadToken, body: { exportFile: "/tmp/export.xml" } })));
  ...
  it("returns 404 when API is disabled", async () => {
    ...
    const res = await POST(ctx(req({ token: importWriteToken, body: { exportFile: "/tmp/export.xml" } })));
...
describe("POST /ap-api/v1/import/wordpress — success path", () => {
  ...
  it("calls importWordPress with the provided exportFile and applyLocal: true", async () => {
    await POST(ctx(req({ token: importWriteToken, body: { exportFile: "/tmp/export.xml" } })));
