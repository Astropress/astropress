...
describe("global privacy: data minimization", () => {
	it("public comment submission does not collect device fingerprint data", () => {
		...
		for (const col of prohibited) {
			expect(
				commentTable,
				`comments must not store ${col} (device fingerprint)`,
			).not.toContain(col);
		}
	});
	...
	it("contact form does not collect more than name, email, and message", () => {
		...
		const columnMatches = [
			...contactTable.matchAll(/^\s{2}(\w+)\s+TEXT/gm),
		].map((m) => m[1]);
		for (const col of columnMatches) {
			expect(
				permitted,
				`contact_submissions column '${col}' is not in the minimal permitted set`,
			).toContain(col);
		}
	});
	...
	it("analytics module only sends data to explicitly configured self-hosted endpoints", () => {
		...
		for (const domain of hardcodedThirdParty) {
			expect(
				analytics,
				`analytics.ts must not hardcode ${domain} as a default endpoint`,
...
describe("global privacy: storage limitation", () => {
	it("admin sessions are time-limited and auto-expired", () => {
		...
		expect(authSrc, "sessions must have a TTL").toMatch(
			/SESSION_TTL_MS|ttl_ms|hours/i,
		);
	...
	it("API tokens have an expires_at column — tokens can have a fixed lifetime", () => {
		...
		expect(
			apiTokensTable,
			"API tokens must support expiry via expires_at",
	...
	it("API tokens have a revoked_at column — tokens can be invalidated before expiry", () => {
		...
		expect(
			apiTokensTable,
			"API tokens must be revocable via revoked_at",
	...
	it("password reset tokens have an expires_at column — reset links cannot be used indefinitely", () => {
		const resetTable =
			schema.match(
				/CREATE TABLE IF NOT EXISTS password_reset_tokens \([\s\S]*?\);/,
			)?.[0] ?? "";
		expect(resetTable, "password_reset_tokens must expire").toContain(
			"expires_at",
		);
	});
	...
	it("user invite tokens have an expires_at column — invitations cannot be accepted indefinitely", () => {
		const inviteTable =
			schema.match(
				/CREATE TABLE IF NOT EXISTS user_invites \([\s\S]*?\);/,
			)?.[0] ?? "";
...
describe("global privacy: security measures for stored data", () => {
	it("passwords are stored as hashes, never in plain text", () => {
		expect(
			schema,
			"schema must not have a plain 'password TEXT' column",
		).not.toMatch(/\bpassword\s+TEXT\b/gi);
	...
	it("crypto utilities use Argon2 or a comparable KDF for password hashing", () => {
		const cryptoUtils = readFileSync(
			path.join(srcRoot, "crypto-utils.ts"),
			"utf8",
		);
		// Must use argon2, bcrypt, or scrypt — not MD5 or raw SHA hashing.
		expect(
			cryptoUtils,
			"passwords must use a proper KDF, not raw SHA hashing",
		).toMatch(/bcrypt|argon2|scrypt/i);
		expect(cryptoUtils, "password hashing must not use MD5").not.toMatch(
			/createHash\s*\(\s*["']md5["']\)/i,
		);
		expect(cryptoUtils, "password hashing must not use raw SHA-1").not.toMatch(
	...
	it("security headers include X-Frame-Options or CSP frame-ancestors — prevents clickjacking", () => {
		const secHeaders = readFileSync(
			path.join(srcRoot, "security-headers.ts"),
			"utf8",
		);
...
describe("global privacy: erasure readiness", () => {
	...
	it("content_revisions cascade-delete when the content record is deleted", () => {
		...
		expect(
			revisionsTable,
			"content revisions must cascade on content deletion",
		).toMatch(/ON DELETE CASCADE/i);
	});
	...
	it("admin users can be suspended (active = 0) without data loss, preserving audit trails", () => {
		...
		expect(
			adminUsersTable,
			"admin_users must have an active flag for non-destructive suspension",
		).toContain("active");
	});
});
...
describe("global privacy: purpose limitation", () => {
		...
		const commentAction = (() => {
			try {
				return readFileSync(
					path.resolve(
						import.meta.dirname,
						"../pages/ap-admin/actions/comment-moderate.ts",
					),
					"utf8",
				);
			} catch {
	...
	it("contact form submissions are stored separately from newsletter subscribers", () => {
		...
		const newsletterTable =
			schema.match(
				/CREATE TABLE IF NOT EXISTS newsletter_subscribers \([\s\S]*?\);/,
			)?.[0] ?? "";
		// If a newsletter table exists, contact form email must not be auto-inserted there
		if (newsletterTable) {
			const bootstrap = readFileSync(
				path.join(srcRoot, "sqlite-bootstrap.ts"),
				"utf8",
			);
			expect(
				bootstrap,
		...
		expect(
			contactTable,
			"contact_submissions must not auto-subscribe to marketing",
		).not.toMatch(/subscribed|opted_in|marketing/i);
	});
});
...
describe("global privacy: cross-border transfer defaults", () => {
	it("default analytics configuration is empty — no data sent to any external service by default", () => {
		...
		expect(
			...
			/analytics:\s*\{\s*type:\s*["'](google|segment|mixpanel|amplitude)/i,
		);
	});
	...
	it("Cloudflare adapter does not send data to third-party analytics by default", () => {
		...
		for (const domain of thirdParty) {
			expect(
				cloudflare,
				`cloudflare adapter must not send data to ${domain}`,
			).not.toContain(domain);
		}
	});
});
	...
	it("no module exports data to advertising networks or data brokers", () => {
		...
		for (const filename of filesToCheck) {
			...
			for (const domain of adNetworkDomains) {
				expect(
					src,
					`${filename} must not reference ad network ${domain}`,
				).not.toContain(domain);
			}
		}
	});
