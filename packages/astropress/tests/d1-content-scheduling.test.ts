...
describe("D1 scheduling — schedulePublish", () => {
  ...
  it("creates an override with scheduled_at when none exists", async () => {
    ...
    const row = db.prepare("SELECT slug, status, scheduled_at FROM content_overrides WHERE slug = ?").get("my-post") as {
      slug: string;
      status: string;
      scheduled_at: string;
    };
  ...
  it("updates existing override's scheduled_at", async () => {
    ...
    const row = db.prepare("SELECT scheduled_at FROM content_overrides WHERE slug = ?").get("my-post") as {
      scheduled_at: string;
    };
    expect(row.scheduled_at).toBe(future);
  });
...
describe("D1 scheduling — cancelScheduledPublish", () => {
  it("clears scheduled_at without changing status", async () => {
    ...
    const row = db.prepare("SELECT status, scheduled_at FROM content_overrides WHERE slug = ?").get("cancel-post") as {
      status: string;
...
describe("D1 scheduling — runScheduledPublishes", () => {
  it("publishes past-due entries and returns count", async () => {
    ...
    const due = db.prepare("SELECT status, scheduled_at FROM content_overrides WHERE slug = ?").get("due-post") as {
      status: string;
      scheduled_at: string | null;
    };
    ...
    const notDue = db.prepare("SELECT status, scheduled_at FROM content_overrides WHERE slug = ?").get("not-due-post") as {
      status: string;
      scheduled_at: string;
    };
