...
export class ApCommandPalette extends HTMLElement {
  ...
  connectedCallback() {
    this.dialog = this.querySelector<HTMLDialogElement>("#cmd-palette")!;
    this.input  = this.querySelector<HTMLInputElement>(".cmd-input")!;
    this.results = this.querySelector<HTMLUListElement>(".cmd-results")!;
  ...
  private onInput = () => {
    ...
    const filtered = q ? this.items.filter((item) => item.label.toLowerCase().includes(q)) : this.items;
    this.selectedIndex = -1;
  ...
  private onInputKeydown = (e: KeyboardEvent) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      if (link) { this.dialog.close(); link.click(); }
    }
  };

  private updateSelection(lis: HTMLLIElement[]) {
    lis.forEach((li, i) => li.setAttribute("aria-selected", i === this.selectedIndex ? "true" : "false"));
