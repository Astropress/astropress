...
import { createRuntimeContentRecord } from "astropress";
import { withAdminFormAction } from "astropress";

export const POST: APIRoute = async (context) =>
  withAdminFormAction(context, { failurePath: "/wp-admin/posts/new" }, async ({ actor, formData, locals, redirect, fail }) => {
    ...
    const slug = typeof result.state === "object" && result.state && "slug" in result.state ? String(result.state.slug) : "";
    return redirect(`/wp-admin/posts/${slug}?created=1`);
  });
