...
export const POST: APIRoute = async (context) =>
  withAdminFormAction(context, { failurePath: "/wp-admin/route-pages" }, async ({ actor, formData, locals, redirect, fail }) => {
    ...
    if (!result.ok) {
      return fail(result.error, `/wp-admin/route-pages${path}`);
    }

    return redirect(`/wp-admin/route-pages${path}?saved=1`);
  });
