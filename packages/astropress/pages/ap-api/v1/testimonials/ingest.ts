...
function mapFormbricksPayload(
	...
	const answers: unknown[] = Array.isArray(responseData?.answers)
		? (responseData.answers as unknown[])
		: [];
	for (const answer of answers) {
		...
		if (headline.includes("name")) result.name = value;
			...
			headline.includes("challenge") ||
			headline.includes("problem")
		)
			result.beforeState = value;
		else if (
			headline.includes("transform") ||
			headline.includes("happen") ||
			headline.includes("experience")
		)
			result.transformation = value;
		else if (
			headline.includes("result") ||
			headline.includes("outcome") ||
			headline.includes("specific")
		)
...
export const POST: APIRoute = async ({ request, locals }) => {
	...
	const allowed = await checkRuntimeRateLimit(
		`testimonial:ingest:${ip}`,
		RATE_LIMIT_MAX,
