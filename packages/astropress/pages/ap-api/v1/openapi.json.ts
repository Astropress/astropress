...
const OPENAPI_SPEC = {
	...
	info: {
		...
		version: "1.0.0",
		description:
			"REST API for programmatic access to Astropress CMS. All endpoints (except /openapi.json) require Bearer token authentication.",
	},
	...
	components: {
		securitySchemes: {
			BearerAuth: {
				...
				scheme: "bearer",
				description:
					"API token created via Admin → API Tokens. Include as: Authorization: Bearer <token>",
			},
		},
		schemas: {
			ContentRecord: {
			...
			PaginatedContent: {
				...
				properties: {
					records: {
						type: "array",
						items: { $ref: "#/components/schemas/ContentRecord" },
					},
					total: { type: "integer" },
					limit: { type: "integer" },
					offset: { type: "integer" },
	...
	paths: {
			...
			get: {
				...
				parameters: [
					...
					{ name: "status", in: "query", schema: { type: "string" } },
					{
						...
						description: "Records per page (alias: per_page)",
					},
					{
						name: "per_page",
						in: "query",
					...
					},
					{
						...
						description: "Zero-based record offset (alternative to page)",
					},
				],
				responses: {
					200: {
		...
		"/content/{id}": {
			...
			delete: {
				summary: "Delete content (archives it)",
				...
				responses: {
					200: {
						...
						headers: {
							"X-Total-Count": {
								schema: { type: "integer" },
								description: "Total number of media assets",
							},
						},
					},
				},
		...
		"/revisions/{recordId}": {
			get: {
				summary: "List revisions for a content record",
