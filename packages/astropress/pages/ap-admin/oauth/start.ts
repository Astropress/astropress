...
export const POST: APIRoute = async (context) =>
	withAdminFormAction(
		...
		async ({ formData, locals, redirect, fail }) => {
			...
			const returnTo =
				String(formData.get("returnTo") ?? "/ap-admin/services").trim() || "/ap-admin/services";
			...
			const provider = getOAuthProvider(domain as IntegrationDomain, providerId);
			if (!provider) {
				return fail("Unknown OAuth provider for this domain.");
			}
			const env =
				(locals as { runtime?: { env?: Record<string, string> } } | null)?.runtime?.env ??
				(typeof process !== "undefined" ? process.env : {});
			const clientId = env[provider.clientIdEnv];
			if (!clientId) {
				return fail(`OAuth provider is not configured (env ${provider.clientIdEnv} is missing).`);
			}
