...
export const POST: APIRoute = async ({ request, cookies, url, locals }) => {
  try {
    ...
    if (url.searchParams.get("logout") === "1") {
      ...
      return createAstropressSecureRedirect("/ap-admin/login", 302, { forceHsts: request.url.startsWith("https://") });
    }
    ...
    return createAstropressSecureRedirect("/ap-admin", 302, { forceHsts: request.url.startsWith("https://") });
  } catch {
