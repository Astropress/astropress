...
export const POST: APIRoute = async ({ request, locals }) => {
	if (!isTrustedStrictRequestOrigin(request)) {
		return createAstropressSecureRedirect(
			"/ap-admin/reset-password?error=1&message=Invalid+request+origin",
			302,
			{
	...
	if (result.ok && result.resetUrl) {
		...
		if (!emailResult.ok) {
			...
			redirectUrl.searchParams.set(
				"message",
				emailResult.error ??
					"Password reset email could not be sent. Verify SMTP or Resend settings, then contact support if delivery is still blocked.",
			);
		} else {
