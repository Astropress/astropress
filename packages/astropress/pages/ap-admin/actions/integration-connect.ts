...
import { connectIntegrationAction, withAdminFormAction } from "@astropress-diy/astropress";
...
export const POST: APIRoute = async (context) =>
	withAdminFormAction(
		...
		async ({ formData, locals, redirect, fail }) => {
			...
			const result = await connectIntegrationAction(locals, {
				domain: domain as IntegrationDomain,
				providerId,
				fields,
			});
			if (!result.ok) {
				return fail(`integration connect failed: ${result.code}`);
			}
			return redirect(
				`/ap-admin/services?connected=${encodeURIComponent(domain)}:${encodeURIComponent(providerId)}`,
			);
		},
	);
