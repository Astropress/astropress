...
export const POST: APIRoute = async (context) =>
	withAdminFormAction(
		...
		async ({ actor, formData, locals, redirect, fail }) => {
			...
			const result =
				kind === "tag"
					? await deleteRuntimeTag(id, actor, locals)
					: await deleteRuntimeCategory(id, actor, locals);
			if (!result.ok) {
			...
			return redirect(
				`/ap-admin/taxonomies?deleted=1&restore_table=${restoreTable}&restore_id=${id}`,
			);
		},
	);
