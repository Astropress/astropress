...
import type { ApiScope } from "astropress/platform-contracts.js";
...
export const POST: APIRoute = async (context) =>
  withAdminFormAction(context, { failurePath: "/ap-admin/api-tokens", requireAdmin: true }, async ({ formData, redirect, fail }) => {
    ...
    const { record, rawToken } = await store.apiTokens.create({ label, scopes });
    // Pass the raw token once via redirect query param — page shows it once then it's gone
    return redirect(`/ap-admin/api-tokens?created=1&tokenId=${encodeURIComponent(record.id)}&rawToken=${encodeURIComponent(rawToken)}`);
  });
