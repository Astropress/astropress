...
export const POST: APIRoute = async (context) =>
	withAdminFormAction(
		...
		async ({ formData, locals, redirect, fail }) => {
			...
			const result = await setActiveIntegrationProviderAction(
				locals,
				domain as IntegrationDomain,
				providerId,
			);
			if (!result.ok) {
				return fail(`set active provider failed: ${result.code}`);
			}
			return redirect(
				`/ap-admin/services?active=${encodeURIComponent(domain)}:${encodeURIComponent(providerId)}`,
			);
		},
	);
